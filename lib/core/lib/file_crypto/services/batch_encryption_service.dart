...
class BatchEncryptionService {
  ...
  ///
  /// Optionally accepts a custom [FileEncryptionService] for testing.
  BatchEncryptionService([FileEncryptionService? encryptionService])
  ...
  Future<BatchOperationSummary> encryptDirectory({
    ...
    if (!await inputDir.exists()) {
      throw EncryptionException('Input directory not found: $inputDirectory');
    }
  ...
  Future<BatchOperationSummary> decryptDirectory({
    ...
    final inputDir = Directory(inputDirectory);
    if (!await inputDir.exists()) {
      throw DecryptionException('Input directory not found: $inputDirectory');
    }
  ...
  Future<BatchFileResult> _decryptSingleFile({
    ...
    try {
      // Read encrypted data to get header info for output filename
      final file = File(inputPath);
      final encryptedData = await file.readAsBytes();

      // Decrypt to get the header with original extension
      final result = await _encryptionService.decryptBytes(
      ...
      final outputFileName =
          '${result.header.uuid}.${result.header.fileExtension}';
      final outputPath = p.join(outputDirectory, outputFileName);

      // Write decrypted content
      await File(outputPath).writeAsBytes(result.decryptedData);
