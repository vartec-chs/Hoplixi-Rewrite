...
abstract final class KeyDerivationService {
  ...
  /// );
  /// // Use keyPair.encryptionKey for AEAD, keyPair.hmacKey for HMAC
  /// // Don't forget to call keyPair.destroy() when done
  /// ```
  static Future<DerivedKeyPair> deriveKeyPair({
    ...
    final encKeyBytes = Uint8List.fromList(
      masterKeyBytes.sublist(0, keyLength),
    );
    final hmacKeyBytes = Uint8List.fromList(
      masterKeyBytes.sublist(keyLength, totalKeyLength),
    );

    // Note: masterKeyBytes from extractBytes() is immutable (SensitiveBytes),
    // so we cannot clear it manually. The GC will handle cleanup.
    // The split keys are stored in new Uint8List instances.
